...
def callback(lcl, _glb):
    ...
    is_solved = (lcl['saved_mean_reward_diff']!=None) and (lcl['saved_mean_reward_diff']<=0.05)
    return is_solved
...
def train(env,save_path,optimisation_factor=1):
  ...
  for n in n_list:
    for space_seed in space_seed_list:
        ...
        with tf.Graph().as_default():
            train(env,save_path+filename+".pkl",optimisation_factor)
        with tf.Graph().as_default():
